...
One of the important tasks regarding cluster analysis, which is directly related to the focal research questions, is setting the parameters necessary to generate biologically relevant GPS location clusters (GLC; hereafter called clusters). Clusters are formed by applying a buffer of x metres around every GPS location. If these buffers overlap (so at a maximum distance of x × 2), they will form clusters which will be saved as one if y number of GPS locations are within this buffer using all locations within the study period. 
...
When performing a subsequent cluster analysis, it is important to check if the most recent cluster file is loaded (Figure 9), as this file will make sure the earlier established cluster IDs remain constant even with additional GPS locations that might increase, combine, or create new clusters. Furthermore, if GPS locations were added to a cluster from a previous cluster analysis, the statement ‘GPS locations added!’ will appear in the ‘Notes’ column. This statement will persist over multiple cluster analysis and has to be removed manually, to reset the ‘Notes’ column. Same is done for clusters that grew together. A statement in the ‘Notes’ column will inform the user which clusters grew together and this statement will persist over the subsequent cluster analyses. That is why **it is important to always download the cluster table as a shape file!** You can optionally automatically mark the state of all clusters from the most recent file as ‘Done’.
